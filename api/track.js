...
// ── Server-side daily promo code (mirrors frontend DAILY_CODES map) ───────────
function getDailyCode() {
  const DAILY_CODES = {
    0: 'FLOW-P',  // Pühapäev (Sunday)
    1: 'FLOW-E',  // Esmaspäev (Monday)
    2: 'FLOW-T',  // Teisipäev (Tuesday)
    3: 'FLOW-K',  // Kolmapäev (Wednesday)
    4: 'FLOW-N',  // Neljapäev (Thursday)
    5: 'FLOW-R',  // Reede (Friday)
    6: 'FLOW-L',  // Laupäev (Saturday)
  };
  // Use Tallinn timezone (UTC+3 in summer, UTC+2 in winter)
  const day = new Date(new Date().toLocaleString('en-US', { timeZone: 'Europe/Tallinn' })).getDay();
  return DAILY_CODES[day] || 'FLOW19';
}
...
<table cellpadding="0" cellspacing="0" border="0" width="600" style="max-width:600px;width:100%;background:#ffffff;border-radius:4px;overflow:hidden;">
  ...
  <tr><td style="background:#ffffff;padding:8px 48px 56px;text-align:center;">
    ...
    <a href="https://online.saloninfra.ee/KSA?booking=new&campaign=kiirtest&services=42499" style="display:inline-block;background:#87BE23;color:#0F0F0F;font-size:14px;font-weight:700;text-decoration:none;padding:16px 40px;border-radius:3px;letter-spacing:0.02em;">Broneeri uuring &rarr;</a>
...
const EMAIL = {
  ET: {
    ...
    ctaUrl: 'https://online.saloninfra.ee/KSA?booking=new&campaign=kiirtest&services=42499',
...
function buildEmailHtml(lang, name, promoCode, result, answers) {
  ...
  const ctaUrl = isGood
    ? `${t.ctaUrl}${promoCode ? '?promokood=' + promoCode : ''}`
    : t.ctaAuditUrl;
  ...
  ET: [
    {
      ...
      build: (code) => buildSeqHtml(
        ...
        `https://online.saloninfra.ee/KSA?booking=new&campaign=kiirtest&services=42499&promokood=${code}`,
